...
px_vols=$(kubectl -n ${namespace} exec service/portworx-service -- /opt/pwx/bin/pxctl volume list -j | jq -r '.[] | "\(.locator.name)"')
...
for vol in $px_vols; do
  if [[ ! " ${k8s_pv_array[@]} " =~ " ${vol} " ]]; then
          kubectl -n ${namespace} exec service/portworx-service -- /opt/pwx/bin/pxctl volume inspect $vol --json | jq -r '.[] | "\(.locator.name) - \(.id) - \(.state) - \(.status) - \(.source.parent) - \(.ctime) - \(.attach_time) - \(.detach_time) - \(.format) - \(.attached_on) - \(.attached_state)"'
  fi
done
